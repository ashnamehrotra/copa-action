...
@test "Check patched image exists" {
    docker images
    id=$(docker images --quiet 'opa:0.46.0-patched')
    assert_not_equal "$id" ""
}
...
@test "Run trivy on patched image" {
    run trivy image --exit-code 1 --vuln-type os --ignore-unfixed -f json -o opa.0.46.0-patched.json 'docker.io/library/opa:0.46.0-patched'
    [ "$status" -eq 0 ]
